...
void nv::fillExtrapolate(int passCount, FloatImage * img, BitMap * bmap, int coverageIndex/*-1*/)
{
	nvCheck(img != NULL);
	nvCheck(bmap != NULL);
    nvCheck(img->depth() == 1);
	...
	const int count = img->componentCount();
		...
		for(int c = 0; c < count; c++)
		{
			...
			for(int y = 0; y < h; y++) {
				for(int x = 0; x < w; x++) {
					...
					const bool west = bmap->bitAt(img->indexClamp(x-1, y, 0));
					const bool east = bmap->bitAt(img->indexClamp(x+1, y, 0));
					const bool north = bmap->bitAt(img->indexClamp(x, y-1, 0));
					const bool south = bmap->bitAt(img->indexClamp(x, y+1, 0));
					const bool northwest = bmap->bitAt(img->indexClamp(x-1, y-1, 0));
					const bool northeast = bmap->bitAt(img->indexClamp(x+1, y-1, 0));
					const bool southwest = bmap->bitAt(img->indexClamp(x-1, y+1, 0));
					const bool southeast = bmap->bitAt(img->indexClamp(x+1, y+1, 0));
					...
					if (num != 0) {
						...
						if (num == 3 && west && northwest && southwest) {
							...
							float total = 0.0f;
							if (west) { average += 1 * channel[img->indexClamp(x-1, y, 0)]; total += 1; }
							if (east) { average += 1 * channel[img->indexClamp(x+1, y, 0)]; total += 1; }
							if (north) { average += 1 * channel[img->indexClamp(x, y-1, 0)]; total += 1; }
							if (south) { average += 1 * channel[img->indexClamp(x, y+1, 0)]; total += 1; }
						
							if (northwest) { average += channel[img->indexClamp(x-1, y-1, 0)]; ++total; }
							if (northeast) { average += channel[img->indexClamp(x+1, y-1, 0)]; ++total; }
							if (southwest) { average += channel[img->indexClamp(x-1, y+1, 0)]; ++total; }
							if (southeast) { average += channel[img->indexClamp(x+1, y+1, 0)]; ++total; }
							
							average /= total;
						}

						channel[img->indexClamp(x, y, 0)] = average;
						newbmap->setBitAt(x, y);
					}
				}
			}
...
void nv::fillVoronoi(FloatImage * img, const BitMap * bmap, int coverageIndex/*-1*/)
{
    nvCheck(img != NULL);
    nvCheck(img->depth() == 1);
        ...
        for( x = 0; x < w; x++ ) {
            ...
            if (sx < w && sy < h)
            {
                if (sx != x || sy != y)	{
                    for(int c = 0; c < count; c++ ) {
                        if (c != coverageIndex) {
                            img->pixel(c, x, y, 0) = img->pixel(c, sx, sy, 0);
                        }
                    }
                }
            }
...
static int countMipmaps(uint w, uint h)
{
    int count = 1;
    while (w != 1 && h != 1) {
	    w = max(1U, w / 2);
...
// @@ Add option with nearest filtering.
// @@ This code has bugs, see one reference implementation here:
// http://http.developer.nvidia.com/GPUGems2/gpugems2_chapter37.html
void nv::fillPullPush(FloatImage * img, const BitMap * bmap, int coverageIndex)
{
    nvCheck(img != NULL);
    nvCheck(img->depth() == 1);

    const uint componentCount = img->componentCount();
    const uint w = img->width();
    const uint h = img->height();
    const uint mipmapCount = countMipmaps(w, h);

    // Build mipmap chain.
    Array<FloatImage *> mipmaps;
    mipmaps.resize(mipmapCount);

    FloatImage * maskedImage = new FloatImage;
    maskedImage->allocate(componentCount + 1, w, h);

    for (uint y = 0; y < h; y++) {
        for (uint x = 0; x < w; x++) {
            for (uint c = 0; c < componentCount; c++) {
                maskedImage->pixel(c, x, y, 0) = img->pixel(c, x, y, 0);
            }
            maskedImage->pixel(componentCount, x, y, 0) = bmap->bitAt(x, y);
        }
    }

    mipmaps[0] = maskedImage;

    int maskChannel = componentCount;

    QuadraticFilter filter;
    for (uint i = 1; i < mipmapCount; i++)
    {
        mipmaps[i] = mipmaps[i-1]->downSample(filter, FloatImage::WrapMode_Clamp, maskChannel);

        // Binary alpha.
        const uint mw = mipmaps[i]->width();
        const uint mh = mipmaps[i]->height();
        for (uint y = 0; y < mh; y++) {
            for (uint x = 0; x < mw; x++) {
                float & alpha = mipmaps[i]->pixel(maskChannel, x, y, 0);
                if (alpha > 0.0f) {
                    alpha = 1.0f;
                }
            }
    ...
    for (uint y = 0; y < h; y++) {
        for (uint x = 0; x < w; x++) {
            ...
            {
                if (c == coverageIndex) {
                    // Do not touch coverage channel.
                    continue;
                }

                float colorSum = 0;

                // Blend mipmaps back to front.
                for (uint m = 0; m < mipmapCount; m++)
                {
                    float color = mipmaps[mipmapCount - m - 1]->sampleLinearClamp(c, fx, fy);
                    float alpha = mipmaps[mipmapCount - m - 1]->sampleLinearClamp(maskChannel, fx, fy);
                    
                    colorSum = colorSum * (1-alpha) + color * alpha;
                }

                img->pixel(c, x, y, 0) = colorSum;
            }
        }
    }
        ...
        for (int c = 0; c < count; c++)
        {
            ...
            for (int yb = 0; yb < h; yb++) {
                for (int xb = 0; xb < w; xb++) {
                    ...
                    {
                        const int idx = img->index(xb, yb, 0);
